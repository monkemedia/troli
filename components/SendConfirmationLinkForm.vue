<template>
  <form
    ...
    <custom-input
      ...
      :label="$t('pages.confirm_account.label')"
      is-required
      type="text"
      data-qa="email box"
    ...
    <div v-if="!success" class="field">
      <div class="control">
        <button-default
          :text="$t('pages.confirm_account.button')"
          :class="{ 'is-loading' : isLoading }"
          is-flip
          data-qa="send confirmation link button"
          type="submit" />
      </div>
    </div>
  </form>
  ...
  export default {
    ...
    methods: {
      async submitEmail () {
        ...
        try {
          response = await axios.post('/api/v1/resend-confirmation-link', this.form)
          this.isLoading = false
          this.alert = [
            {
              status: 200,
